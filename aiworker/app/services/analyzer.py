...
from app.services.telegram_news import TelegramNewsService, TelegramNewsContext
...
def _resolve_safety_tag(
    ...
    if telegram_news.risk_tag == "BLOCK":
        return "BLOCK"

    if snapshot.isFriday and snapshot.isUsRiskWindow:
        return "BLOCK"
    if volatility_expansion >= 1.20:
        return "BLOCK"
    if telegram_news.risk_tag == "CAUTION":
        return "CAUTION"
...
def _resolve_alignment_score(
    ...
    score = confidence
    if snapshot.session.upper() in {"LONDON", "NEW_YORK", "EUROPE"}:
        score += 0.04
...
def _build_summary(snapshot: MarketSnapshot, volatility_expansion: float, telegram_news: TelegramNewsContext) -> str:
    return (
        ...
        f"usRisk={snapshot.isUsRiskWindow}, friday={snapshot.isFriday}, "
