...
public static class MonitoringEndpoints
{
    ...
    private static string DescribeEvent(RuntimeTimelineEvent item)
    {
        return item.EventType switch
        {
            ...
            "TIME_MAPPING_CONFLICT" => "Time integrity: Clock vs SessionEngine session/phase conflict; time-sensitive rules may be degraded.",
            "SESSION_SOURCE_SELECTED" => "Time integrity: Canonical session source (SessionEngine) and chosen session/phase for cycle.",
            "REGIME_RECONCILED" => "Regime: Master reconciled regime, finalTradeability, dominantBlocker, downgradedSignals.",
            "WATERFALL_VETO_APPLIED" => "Regime: Pattern detector WaterfallRisk BLOCKED; authoritative; shouldBlockNewTrades=true.",
            "WATERFALL_VETO_CLEARED" => "Regime: Waterfall veto cleared by rebuild-proof path.",
            "SWEEP_RECLAIM_STATUS" => "Structure: Sweep/reclaim proof chain (sweepFound, reclaimCloseFound, levels).",
            "RETEST_STATUS" => "Structure: Retest hold and compression diagnostics.",
            "COMPRESSION_STATUS" => "Structure: Compression bars, base/lid, expansion count.",
            "CANDIDATE_PROMOTION_REASON" => "Structure: Why candidate was promoted to valid setup.",
            "WHY_ZONE_NOT_ARMED" => "Structure: Why nearest legal buy zone is informational only, not armable.",
            "VERIFY_CONTEXT_BUILT" => "Governance: VERIFY engine context (credibility, zone alignment).",
            "VERIFY_ALIGNMENT_RESULT" => "Governance: VERIFY alignment result (ALIGNED/NONE, advisory weight).",
            "NEWS_CONTEXT_BUILT" => "Governance: NEWS engine context (hazard, regime, rail, minutes to clean).",
            "HAZARD_WINDOW_EVALUATED" => "Governance: Hazard window active flag from DB.",
            "RAIL_PERMISSION_SET" => "Governance: Rail-A/Rail-B permission from NEWS; never overrides FAIL/waterfall/hazard.",
            "TABLE_COMPILED" => "Table: TABLE compiler output (isValid, orderType, entry, tp, grams, expiry, template).",
            "TABLE_VALIDATED" => "Table: VALIDATE engine result (expiry/tp/size/regime/capacity/safety pass).",
            "EXECUTION_PAYLOAD_BUILT" => "Execution: TABLE-only payload built for MT5 or approval queue.",
            "LOCAL_FINAL_RECHECK_PASSED" => "Execution: Capital and exposure gates passed; order may proceed.",
            "MT5_PENDING_ORDER_SUBMITTED" => "Execution: Pending order sent to MT5 queue.",
            "LEDGER_SOURCE_APPLIED" => "Ledger: Source (GetState), cash/gold/deployable.",
            "LAST_SLIP_USED" => "Ledger: Last slip applied (BUY_FILL/SELL_FILL); source of truth.",
            "CAPITAL_BUCKETS_COMPUTED" => "Ledger: Cash, reserved pending, deployable after reserved.",
            "DEPLOYABLE_CAPACITY_COMPUTED" => "Ledger: Deployable cash and reserved capital explanation.",
            "RESERVED_CAPITAL_EXPLAINED" => "Ledger: Reserved for pending orders, count, note.",
