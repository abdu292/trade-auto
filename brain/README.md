...
- `Security:RelaxIpAllowList=true` (temporary for dynamic IP clients)
- `Security:ApiKeyHeaderName=X-API-Key`
- `Security:ApiKey=dev-local-change-me`
- `Security:AllowedIps=[]`

When using dynamic client IPs (for example MT5 on broker/VPS), keep `Security:ApiKey` strong and rotate it regularly while `RelaxIpAllowList=true`. Re-enable strict IP filtering later by setting `Security:RelaxIpAllowList=false` and filling `Security:AllowedIps`.
